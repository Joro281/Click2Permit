...
function getNotificationCount($lastCheckTimestamp) {
    ...
    $countSql = "SELECT COUNT(*) AS new_records FROM service WHERE created_at > '$lastCheckTimestamp'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        $deleteSql = "DELETE FROM service WHERE ID = '$deleteId'";
        $conn->query($deleteSql);
    } elseif (isset($_POST['approve_id'])) {
        $approveId = $_POST['approve_id'];
        // Update the request status in the database to 'Approved'
        $approveSql = "UPDATE service SET Request_Status = 'Approved' WHERE ID = '$approveId'";
        ...
        $declineSql = "UPDATE service SET Request_Status = 'Declined' WHERE ID = '$declineId'";
...
<div class="navbar">
    ...
    $messagesSql = "SELECT * FROM service WHERE created_at > '$lastCheckTimestamp'";
    ...
    while ($messageRow = $messagesResult->fetch_assoc()) {
        ...
        echo '<strong>' . $messageRow['Firstname'] . ' ' . $messageRow['Lastname'] . '</strong> added to Service Database<br>';
