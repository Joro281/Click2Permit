<?php
session_start(); // Add this line to initialize sessions
...
// Function to get the count of new records since the last check
function getNotificationCount($lastCheckTimestamp) {
    global $conn;

    $countSql = "SELECT COUNT(*) AS new_records FROM newemployed WHERE created_at > '$lastCheckTimestamp'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        $deleteSql = "DELETE FROM newemployed WHERE ID = '$deleteId'";
        $conn->query($deleteSql);
    }
}

$sql = "SELECT * FROM newemployed";
...
<html>
<head>
    <link rel="stylesheet" href="css/ViewCOE.css" />
    <style>
            body {
                margin: 0;
            }
...
<body>
    ...
    $messagesSql = "SELECT * FROM newemployed WHERE created_at > '$lastCheckTimestamp'";
    ...
    while ($messageRow = $messagesResult->fetch_assoc()) {
        ...
        echo '<strong>' . $messageRow['Firstname'] . ' ' . $messageRow['Lastname'] . '</strong> added to Newly Employed Database<br>';
        ...
        <div class="div">
            <div class="overlap">
                ...
                <div class="overlap-2">
                    ...
                    <div class="text-wrapper-3">LIST OF NEWLY EMPLOYED USERS</div>
    ...
    <script>
        ...
        function goBack() {
            window.location.href = "http://localhost:3000/ViewDocReq1.php";
        }
