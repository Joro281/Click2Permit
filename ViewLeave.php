...
function getNotificationCount($lastCheckTimestamp) {
    ...
    $countSql = "SELECT COUNT(*) AS new_records FROM leave1 WHERE created_at > '$lastCheckTimestamp'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        $deleteSql = "DELETE FROM leave1 WHERE ID = '$deleteId'";
        $conn->query($deleteSql);
    } elseif (isset($_POST['approve_id'])) {
        $approveId = $_POST['approve_id'];
        // Update the request status in the database to 'Approved'
        $approveSql = "UPDATE leave1 SET Request_Status = 'Approved' WHERE ID = '$approveId'";
        ...
        $declineSql = "UPDATE leave1 SET Request_Status = 'Declined' WHERE ID = '$declineId'";
...
<div class="navbar">
    ...
    $messagesSql = "SELECT * FROM leave1 WHERE created_at > '$lastCheckTimestamp'";
    ...
    while ($messageRow = $messagesResult->fetch_assoc()) {
        ...
        echo '<strong>' . $messageRow['Firstname'] . ' ' . $messageRow['Lastname'] . '</strong> added to Leave Database<br>';
    ...
    <script>
        ...
        var popup = document.getElementById('notification-popup-' + id);
