...
function getNotificationCount($lastCheckTimestamp) {
    ...
    $countSql = "SELECT COUNT(*) AS new_records FROM coe2 WHERE created_at > '$lastCheckTimestamp'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['delete_id'])) {
        ...
        $deleteSql = "DELETE FROM coe2 WHERE id = '$deleteId'";
        $conn->query($deleteSql);
    } elseif (isset($_POST['approve_id'])) {
        $approveId = $_POST['approve_id'];
        // Update the request status in the database to 'Approved'
        $approveSql = "UPDATE coe2 SET Request_Status = 'Approved' WHERE id = '$approveId'";
        ...
        $declineSql = "UPDATE coe2 SET Request_Status = 'Declined' WHERE id = '$declineId'";
...
// Query to fetch data from the 'coe2' table
$sql = "SELECT * FROM coe2";
...
<body>
    ...
    $messagesSql = "SELECT * FROM coe2 WHERE created_at > '$lastCheckTimestamp'";
    ...
    while ($messageRow = $messagesResult->fetch_assoc()) {
        ...
        echo '<strong>' . $messageRow['Firstname'] . ' ' . $messageRow['Lastname'] . '</strong> added to Coe Database<br>';
    ...
    <script>
        ...
        function goBack() {
            window.location.href = "http://localhost:3000/adminemployed2.php";
        }
